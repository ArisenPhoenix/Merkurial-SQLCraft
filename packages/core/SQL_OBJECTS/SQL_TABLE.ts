...
export default class TABLE extends SQL_BASE {
  ...
  moveRawToConfirmed = (rawRow: Row) => {
    // Remove the matching raw row by primary key
      this.rawRows = this.rawRows.filter(row => row[this.primaryKey] !== rawRow[this.primaryKey]);
            
      // Push to confirmed sqlRows
      const confirmedSQL = new SQL_ROW (
        rawRow,
        this.tableName,
        this.tableSchema,
        this.callAddress,
      ...
      this.sqlRows = [...this.sqlRows, confirmedSQL];
  }
